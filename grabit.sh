...
show_file_content "tailwind.config.js"  # Updated location for tailwind config
...
# Large files
output "=== Large Files (Top 5) ===\n"
output "$(du -ah . | sort -rh | head -n 5)\n"

# File types summary (excluding .git directory)
output "=== File Types Summary ===\n"
output "$(find . -type f ! -path './.git/*' | sed -n 's/..*\.//p' | sort | uniq -c | sort -rn)\n"

# Recently modified files
output "=== Recently Modified Files ===\n"
output "$(find . -type f ! -path './.git/*' -printf '%T+ %p\n' | sort -r | head -n 5)\n"

# Project type detection
output "=== Project Type Detection ===\n"
if [ -f package.json ]; then
    output "- Node.js project\n"
fi
if [ -f Gemfile ]; then
    output "- Ruby on Rails project\n"
fi
