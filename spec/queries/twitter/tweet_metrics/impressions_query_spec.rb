  ...
  describe '#impression_counts_per_day' do
    ...
    context 'when there are tweets without impressions' do
      before do
        ...
        1.upto(7) do |n|
          tweet = create(:tweet, identity:, twitter_created_at: n.days.ago)
          create(:tweet_metric, tweet:, pulled_at: n.days.ago, impression_count: 0)
        end
      end
    ...
    context 'when there are tweets from different days with varying impressions' do
      before do
        ...
        3.times do
          ...
          create(:tweet_metric, tweet:, pulled_at: 1.day.ago, impression_count: 300)
        end

        # Create a tweet from 2 days ago with lower impressions
        tweet = create(:tweet, identity:, twitter_created_at: 2.days.ago)
        create(:tweet_metric, tweet:, pulled_at: 2.days.ago, impression_count: 100)
      end
