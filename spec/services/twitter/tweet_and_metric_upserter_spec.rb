...
RSpec.describe Twitter::TweetAndMetricUpserter do
  ...
  context 'when a user is not provided' do
    let(:subject) { described_class.new(tweet_data: tweet_data) }
    context 'when the tweet does not exist' do
      it 'unable to determine the user from the tweet data' do
    ...
    fcontext 'when the tweet exists' do
      ...
      it 'updates the tweet and metric' do
        tweet_metric = tweet.tweet_metrics.last
        tweet_metric_updated_at = tweet_metric.updated_at
        expect { subject.call }.to change {tweet.reload.updated_at}
        expect(tweet_metric_updated_at).not_to eq(tweet_metric.reload.updated_at)
      end
    end
  end
