...
RSpec.describe Twitter::TweetMetricsRefresher, :vcr do
  ...
  let(:batch_size) { 2 }
  let(:subject) { described_class.new(user:, batch_size:) }
  ...
  xcontext 'when there are outdated tweets' do
    let!(:updatable_tweets) do
      ...
      valid_tweet_ids.each do |id|
        tweets << create(:tweet, identity:, twitter_id: id, updated_at: 25.hours.ago,
                                 twitter_created_at: random_last_7_days)
      end
      tweets
    end

    let!(:outdated_tweet) do
      create(:tweet, identity:, twitter_created_at: 8.days.ago, updated_at: 25.hours.ago)
    end
