...
RSpec.describe PublicPageService do
  ...
  let(:service) { PublicPageService.new(user: user, current_admin_user: admin_user) }
  ...
  describe '#store_impressions_graph_data' do
    let(:service_with_admin) { PublicPageService.new(user: user, current_admin_user: admin_user) }
    let(:service_without_admin) { PublicPageService.new(user: user, current_admin_user: nil) }
  ...
  describe '#store_top_posts' do
    let!(:top_tweets) do
      tweets = create_list(:tweet, 5, identity: identity)
      tweets.map { |tweet| create(:tweet_metric, tweet: tweet) }
    end

    before do
      allow(Twitter::TweetMetricsQuery).to receive(:new).with(user: user).and_return(double('query', top_tweets_for_user: top_tweets))
    end

    it 'assigns the top tweets to @top_posts' do
      service.send(:store_top_posts)
      expect(service.instance_variable_get(:@top_posts)).to match_array(top_tweets)
    end
  end
end
