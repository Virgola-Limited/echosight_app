...
RSpec.describe Twitter::NewTweetsFetcherJob do
  ...
  describe '#perform' do
    ...
    context 'when the user can be found and the batch exists' do
      before do
        allow(Twitter::NewTweetsFetcher).to receive(:new).with(user: user, api_batch_id: api_batch.id).and_call_original
      ...
      it 'does not enqueue ExistingTweetsUpdaterJob if no tweets exist for the user' do
        ...
        subject.perform(user.id, api_batch.id)
      end
      ...
      context 'when there are tweets for the user in the batch' do
        before do
          ...
          allow(api_batch.tweets).to receive(:exists?).with(identity_id: user.identity.id).and_return(true)
        end
